...
- **Telling the user.** An in-app card on the Overview until the changes are opened, a Changes screen (history, one row per changed fact opening to the plain sentence, what it could mean, and the technical before/after), and an optional local notification. The notification is opt-in: the app invites once, in context, after the first baseline exists; the system permission prompt appears only when the user says yes.
