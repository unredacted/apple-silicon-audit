...
**2b — Deliberate tag-check fault (macOS and the CLI only, behind a flag).** No Mach exception handler: the isolation boundary is a **child process of the same binary**. `silicon-audit self-test --fault` (or `export --fault-test`) re-executes itself with a hidden flag; the child `malloc`s 32 bytes and stores one granule past the block, into the neighbour's tag. Measured on the M5 Mac: hardened, the kernel kills the child with **SIGKILL**; its crash report reads `EXC_BAD_ACCESS` / `EXC_ARM_MTE_TAGCHECK_FAIL` at the stored address, termination namespace `MTE_FAIL`, with the faulting frame at the store itself (delivered synchronously, before the child's next instruction); unsigned, the store lands and the child exits 0. Fact `self_test.tag_check_fault`, "Tag mismatch stops this app": `present` (killed by a signal, `probe.child_signal`), `not_present` (survived, `probe.child_exit_status`), `error` (could not spawn). Each run leaves one crash report for the child in `~/Library/Logs/DiagnosticReports`. The app never runs 2b: iOS cannot spawn processes and a fault inside the app would be the very thing to avoid; the CLI flag is the developer toggle the original text asked for. `checked-allocations.soft-mode` was not needed.
