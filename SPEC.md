...
**Capability bitmask**
`caps` (`kind: bitmask`, variable width; 12 bytes in the macOS 26 fixture, 13 bytes on the audited newer kernel). Decode with `CAP_BIT_*` and cross-check against the individual `FEAT_*` keys; any disagreement is a bug or a finding and is exported as such.
...
- **One structure at two depths, one data model.** The *Overview* is the home screen and speaks to non-specialists: a fixed set of plain-language topics (memory tagging hardware, Apple's Memory Integrity Enforcement statement, pointer authentication, branch target protection, speculation hardening, constant-time computing, Apple's documented kernel protections, and on macOS whether tagging is active right now), each with a verdict word, one sentence, and where the answer came from ("Measured on this device" or "Apple's documentation, <date>"). Every topic drills down to the facts behind it and from there to the raw reading, so the simple layer never hides provenance, it only reorders it. The fact-level view is reached by navigation, never by a mode switch: on compact widths a "Look deeper" group at the end of the Overview opens *All readings* (every category on three shelves: security protections, chip and instruction sets, device and system), Apple's documentation, *How this was measured*, and *About the data*; on regular widths the same destinations are the sidebar, always visible, so the sidebar never changes shape underneath the reader.
- **Summary first.** One headline card: the chip (or the device when the chip cannot be named), the device and OS build, and how the Overview's checks came out ("6 yes · 2 partial"). The SoC inference chain, core family, collection method, and "N of M security flags reported present" belong to the *How this was measured* screen, one tap away, so the card never opens with jargon. Categories follow the §4.3 order with memory tagging first. Tapping a fact opens a detail view showing the raw key, value, type, byte length, one plain-English sentence on what it means, and the provenance chain.
...
### 6.5 Change monitoring

A reading is a point in time; the useful question is whether it moves. The app therefore keeps a baseline and notices changes:

- **Baseline.** The first report on a device becomes the baseline, stored as full JSON in Application Support alongside a small state file (baseline date, last check, recorded changes). A report from a different device identity (a restored backup) replaces the baseline silently.
- **Checks.** On launch and on return to the foreground (at most every 15 minutes), and periodically in the background through `BGAppRefreshTask` (iOS, iPadOS, tvOS, visionOS, macOS) or `WKApplication.scheduleBackgroundRefresh` (watchOS), both handled by SwiftUI's `backgroundTask(.appRefresh)`. The system decides the cadence; the app asks for roughly six hours.
- **Diff.** `ReportDiff` in core compares measured facts only (documented and inferred facts change with data updates, not with the device). A fact whose state differs, a non-flag value that differs, and a fact that appears or disappears are each one change; security relevance comes from the category or the inventory's `security_relevant` flag (the export omits it, so the inventory supplies it for decoded baselines). Live gauges (`vm.mte.*`) and the keys that spell the OS build are excluded from the fact list; a build change is reported once as its own event. Every check then advances the baseline to the fresh report, so each change is reported exactly once.
- **Telling the user.** An in-app card on the Overview until the changes are opened, a Changes screen (history, one row per changed fact opening to the plain sentence, what it could mean, and the technical before/after), and an optional local notification. On iPhone and iPad the app asks the system for notification permission at first launch, because the phone also receives Apple Watch deliveries while it is asleep and those post a notification too; elsewhere the app invites once, in context, after the first baseline exists, and the system prompt appears only when the user says yes.
- **Copy at two depths.** "Memory tagging hardware was reported on and now reports off." then "A protection the kernel used to report is no longer reported. A kernel can mask a feature it has not enabled, so this says the kernel stopped reporting it, not that the silicon lost it. The system was not updated between the two readings, so this did not come from an update." then `present → not_present`, the key, both builds and both timestamps.
- **Honest limits, stated in the app.** The monitor sees only what the kernel exposes. Operation Triangulation (2023, CVE-2023-38606) bypassed the hardware kernel-memory protection on A12–A16 devices by writing to undocumented MMIO registers of the GPU coprocessor, a feature Kaspersky believes was meant for debugging or factory test; nothing kernel-visible changed and no sysctl reading would have moved. The monitoring screen says so. A quiet monitor is evidence, not proof.
- **CLI.** `silicon-audit diff <baseline.json> [current.json]` runs the same engine on two exports (or one export against a live audit), exits 1 when facts changed and 2 when the devices differ.
...
- **Export.** Use `WCSession` to transfer the full JSON export to the companion iOS app, which stores it (Application Support, one file per device identity and OS build, newest wins), browses it with the same views as its own report, and offers the share sheet and file export. Use `transferFile` (queued, reliable, background) rather than `transferUserInfo`; the latter's payload ceiling is undocumented and is hit in practice by dictionaries in the tens of kilobytes, which the ~70 KB full export exceeds. `transferFile` carries the uncompressed JSON without trouble, so no gzip layer. `sendMessage` requires reachability and is not appropriate. Measured on hardware (M0): delivery within seconds with the phone unlocked, byte-identical.
...
- **Self-test facts** (schema 1.1.0, §11): a measured fact may carry `probe` instead of `raw` (`oneOf`), with `discovered_by: self_test`. `probe` has `method` (`tagged_pointer_observation` | `child_process_tag_mismatch_store`), `entitlement` (`declared` | `not_declared` | `unknown`), and per method `samples`/`tagged`/`distinct_tags` or `child_signal`/`child_exit_status`. Only the two self-test ids may use this form (`discovered_by: self_test`, category `enforcement`), and each method has its own required fields. Versioning policy: within 1.x, a **minor** bump means 1.x readers read every older 1.x file unchanged, while a strict older validator or the older Swift model may reject a newer file (here: 1.0.0 rejects `probe` and the `self_test` value). Consumers must accept files whose `schema_version` is at most their own; a **major** bump is reserved for changes that break reading older files.
- **Compact variant** (`"variant": "compact"`): the security-relevant measured facts (memory tagging, pointer authentication, control flow, speculation, constant time, capability bitmask, OS tagging) plus identity/context and legacy-alias rows and all `unrecognized_keys`, no display names or descriptions, deflate-compressed and Base45-encoded (RFC 9285) for QR (tvOS) and watch `ShareLink`. ISA/SIMD and x86 rows are full-export only so the payload stays under QR capacity (measured: 3,027 characters for an M5 Mac). Base45's alphabet is exactly QR's alphanumeric character set and CoreImage's `CIQRCodeGenerator` encodes it in alphanumeric mode, so capacity is the alphanumeric-mode figure (4,296 characters at error-correction level L, 3,391 at level M; the generator returns nil rather than truncating beyond it); the tvOS app renders level M, and `ExportTests` keeps the payload under 3,300 characters. The companion app or the CI tool expands it against the same `known_keys_version`.
...
Conflict handling: when two submissions for the same `(identity, os_build)` disagree on a measured fact, do not silently pick one. Surface the disagreement in the generated matrix. "Measured fact" includes every entry of `unrecognized_keys` (§4.1); those have `kind: unknown` and always read as `value`, so for them the raw value is compared too, and a key one submission walked but another did not is a conflict. Counts and bitmasks are compared by state only: core count and memory size legitimately differ between configurations of the same identity. Disagreements usually mean a bug in the app, an environment flag that slipped through, or something genuinely interesting. Submissions for the same `identity` on different `os_build`s that disagree are not conflicts; they are the OS-version boundary the app exists to detect, and the matrix shows them side by side.

Matrix rendering rules: a `FEAT_*` column whose key the kernel does not register (`key_absent`) falls back to the legacy `armv8_*` alias the inventory maps to it (§4.3) and is marked as read through the alias, so an older PAC-capable device does not appear to lack a comparable PAuth report. Documented rows are Apple's table as the app's bundled data understood it at export time; when a device has several results, the documented table shows the one annotated with the newest documentation data (`collection.data_versions.documented_matrix`, else the newest `source.verified`) and says how many results and distinct data snapshots it stands for. The `main` regenerate-and-publish job is serialized with a workflow concurrency group and rebases its generated commit if `main` moved, so overlapping merges cannot leave a result uncommitted or unpublished.
...
A hardware capability flag doesn't prove the OS turned enforcement on for this process. Two probes, in order of risk; both implemented in Phase 9 (2026-09-10) and both **measured, per-process facts**: they describe the build that ran them (its entitlements), never the device. Category `enforcement`; exported with a `probe` object instead of `raw` (§8, schema 1.1.0); `discovered_by: self_test`. The results database never tabulates them per device or treats them as conflicts.

**Entitlements.** Apple's Enhanced Security capability: `com.apple.security.hardened-process` (true), `com.apple.security.hardened-process.enhanced-security-version-string` ("1"; Xcode 26.4+ guidance, the integer `enhanced-security-version` also works), and `com.apple.security.hardened-process.checked-allocations` (true; memory tagging, hardware A19+/M5+). Measured on macOS 26 (Mac17,7, 25G83): ad-hoc signing is enough, no provisioning profile or Apple approval is involved, and App Sandbox coexists with them. The app has `DebugHardened`/`ReleaseHardened` configurations (scheme "SiliconAudit Hardened") that add them and set the Info.plist marker `SiliconAuditEnhancedSecurity` to YES; `Scripts/sign-hardened.sh` re-signs the CLI with `Configs/hardened.entitlements`. Whether the running process declares checked-allocations is read with `SecTaskCopyValueForEntitlement` on macOS and from the marker elsewhere (`declared` / `not_declared` / `unknown`, carried in `probe.entitlement`).

**2a — Tagged-pointer observation (safe; runs on every live audit).** `TaggedPointerProbe` allocates 65 heap blocks of 13 sizes (16 B to 1 MiB, five rounds) and reads bits 59:56 of each pointer, then frees them. Fact `self_test.tagged_pointers`, "Memory tagging active for this app": `present` when any block came back tagged (measured hardened on the M5 Mac: 55 of 65 tagged, 14–16 distinct tag values; large blocks stay untagged), `not_present` when none did (measured unsigned: 0 of 65), `not_applicable` when the kernel's own `FEAT_MTE4`/`FEAT_MTE` reads off or absent, so no process could be tagged. The description says whether the build declared the entitlement. It performs no invalid access and cannot fault. It still does not prove synchronous mode. Fixture runs (`--fixture`) carry no self-test: the CLI's process has nothing to do with the recorded device.

**2b — Deliberate tag-check fault (macOS and the CLI only, behind a flag).** No Mach exception handler: the isolation boundary is a **child process of the same binary**. `silicon-audit self-test --fault` (or `export --fault-test`) re-executes itself with a hidden flag; the child `malloc`s 32 bytes, announces the store on its stdout, stores one granule past the block (into the neighbour's tag), and reports back if it survives. Public API cannot read the kernel's exit reason, so the verdict is strict: **SIGKILL after the announcement and before the survival report** is consistent with a tag-check kill (`present`), but an external SIGKILL cannot be ruled out; confirm the cause in the crash report. A normal exit after the survival report is `not_present` for this access only, since adjacent allocations may share a tag; any other signal (a sanitizer's SIGABRT, Guard Malloc's SIGSEGV, an outside signal), a missing announcement, or an odd exit status is inconclusive (`error`). Measured on the M5 Mac: hardened, the kernel kills the child with **SIGKILL**; its crash report reads `EXC_BAD_ACCESS` / `EXC_ARM_MTE_TAGCHECK_FAIL` at the stored address, termination namespace `MTE_FAIL`, with the faulting frame at the store itself (delivered synchronously, before the child's next instruction); unsigned, the store lands and the child exits 0. Fact `self_test.tag_check_fault`, "Tag mismatch stops this app": `present` (killed by a signal, `probe.child_signal`), `not_present` (survived, `probe.child_exit_status`), `error` (could not spawn). A run that crashes normally leaves a crash report for the child in `~/Library/Logs/DiagnosticReports`. The app never runs 2b: iOS cannot spawn processes and a fault inside the app would be the very thing to avoid; the CLI flag is the developer toggle the original text asked for. `checked-allocations.soft-mode` was not needed.

This is the only measured evidence of enforcement in the project, and it costs one child process. Phase 1 still has standalone value without it.
...
**M7 — Optional enforcement self-test.** 2a first, then 2b. Done in Phase 9: both measured on Mac17,7 / 25G83; see §11.
