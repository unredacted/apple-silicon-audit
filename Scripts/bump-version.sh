...
perl -pi -e "s/(\"version\": \")[0-9.]+(\")/\${1}$VERSION\${2}/" Tools/generate-matrix/package.json
grep -q "^## $VERSION" CHANGELOG.md || perl -0pi -e "s/^## Unreleased\n/## Unreleased\n\n## $VERSION — $(date +%Y-%m-%d)\n/m" CHANGELOG.md
echo "version $VERSION, build $BUILD: project.yml, SiliconAuditCore.swift, Tools/generate-matrix/package.json, CHANGELOG.md"
