...
81. **`ReportListContent`** (now `OverviewContent`, item 117) is the one list body used by the phone's own report, a received watch report and the watch itself, so the three never drift. (§6.4)
...
## M. From Phase 9 implementation (enforcement self-test)

100. **Entitlements need no approval.** An ad-hoc signed binary with `hardened-process`, `enhanced-security-version-string` ("1"; the integer `enhanced-security-version` behaves identically) and `checked-allocations` gets tagged heap pointers on macOS 26 / Mac17,7 (55 of 65 sampled blocks, 16 distinct tag values). Without them every tag is zero. App Sandbox coexists with them; the hardened macOS app built, launched and stayed alive with the Simulator-free Overview for 20 s. (§11)
101. **2a as designed.** `TaggedPointerProbe` reads bits 59:56 of 65 `malloc` results across 13 size classes; blocks of 64 KiB and 1 MiB come back untagged, so "present if any tagged" rather than "if most". Gated by the kernel's own MTE flag to `not_applicable` where no process could be tagged. Fixture audits carry no self-test. (§11)
102. **2b without a Mach exception handler.** A child process of the same binary performs the tag-mismatch store; the parent reads `Process.terminationReason`. Hardened: SIGKILL with `EXC_ARM_MTE_TAGCHECK_FAIL` on the store instruction itself (termination namespace `MTE_FAIL`), so the check is synchronous. Unsigned: the store lands and the child exits 0. No soft mode was needed, and the app never runs it (iOS cannot spawn; the CLI flag is the developer toggle). Each run leaves one crash report for the child. (§11)
103. **Export shape.** Schema 1.1.0 adds `probe` as an alternative to `raw` for measured facts (`oneOf`) and `self_test` to `discovered_by`; additive, so 1.0.0 readers keep working. The results generator excludes `probe` facts from conflict detection and shows them in the per-result notes only: two apps on one device may legitimately differ. (§8, §9)
104. **Reading one's own entitlement.** `SecTaskCopyValueForEntitlement` on macOS; elsewhere an Info.plist marker `SiliconAuditEnhancedSecurity` that the hardened configurations set. Xcode's `INFOPLIST_KEY_*` build settings accept only Apple's known keys, so the app now uses an XcodeGen-generated Info.plist (gitignored) with `$(SILICON_AUDIT_ENHANCED_SECURITY)` substitution. (§6.1)
105. **Verified / not verified.** CLI: unsigned → both facts `not_present`; hardened → both `present`; live and hardened exports validate against the schema; 68 tests pass; CI runs the self-test unsigned and hardened on its VM runner (where both read `not_applicable`). Not verified: the hardened *iOS* build on a device (the phone is in Lockdown Mode; nothing in this phase needs it), and the Overview verdict text on a real screen (the Mac's screen cannot be captured). (§14)

From the Phase 9 pull-request review (Codex, 2026-09-10):

106. **Strict fault verdict.** Any signal used to count as a tag-check kill. The public SDK has no way to read the kernel's exit reason (the `PROC_PIDEXITREASONBASICINFO` flavor is not declared), so the child now announces the store on stdout before making it and reports back if it survives; only SIGKILL between those two markers is `present`, every other ending is `error` ("inconclusive: …"). `FaultTest.classify` is pure and tested for SIGABRT, SIGSEGV, SIGBUS, SIGTERM, SIGKILL-before-announcement, and odd exit codes. (§11)
107. **Schema tightened.** The `probe` alternative is allowed only for the two self-test ids with `discovered_by: self_test` and category `enforcement`; ordinary facts must carry `raw` and a walk/known-list discovery. Each method has its own required fields (`samples`/`tagged`/`distinct_tags`, or `child_signal` for `present` and `child_exit_status` for `not_present`, mutually exclusive); the generator additionally rejects `tagged > samples`, which JSON Schema cannot express, and keys its conflict exclusion on `discovered_by` rather than the mere presence of `probe`. Six negative fixtures rejected. (§8, §9)
108. **Hardware gating uses both flags.** `not_applicable` now requires every readable MTE flag (FEAT_MTE4, FEAT_MTE) to read off/absent and none to read on; MTE4 `key_absent` with MTE `present` is a real `not_present`. (§11)
109. **Versioning honesty.** 1.1.0 files with self-test facts are rejected by strict 1.0.0 validators and the 1.0.0 Swift model; §8 now states the policy (minor: newer readers read older files; consumers accept `schema_version` ≤ their own; major reserved for breaking reads of old files) instead of claiming 1.0 readers ignore the field. (§8)
...
117. **One structure, no mode switch.** The Overview/Details segmented control (item 76) confused both audiences: the sidebar changed shape with the mode, the compact screen's toolbar carried three controls, and the summary card opened with kernel targets and core families. The redesign keeps both depths but reaches the second by navigation: the Overview is the home screen with a chip-first card and a verdict tally; "Look deeper" (compact, watch) or the sidebar (regular) opens *All readings* on three shelves, Apple's documentation, the new *How this was measured* screen (identity chain, collection, totals), and About the data. Fact links carry their own `Report`, so a watch report browsed on the phone can no longer resolve a fact against the phone's own report. `-presentationMode` is gone; `-initialSelection` accepts category ids and `__measurement__` in addition to the earlier routes. (§6.4, §10)

118. **Change monitoring (§6.5).** User request: check periodically, record the state, notify on change with words for both audiences, prompted by Operation Triangulation. Checked against Kaspersky's write-up: CVE-2023-38606 bypassed PPL through undocumented GPU-coprocessor MMIO registers (likely a debug or factory-test feature) and changed nothing kernel-visible, so the feature is framed as "what the kernel reports moved", with the limit stated in the app. Design: measured facts only; live gauges and build keys excluded; a build change is one event; the baseline advances every check so a change is reported once; a different device identity resets silently. Background refresh through SwiftUI `backgroundTask(.appRefresh)` on every platform (BGTaskScheduler off the watch, WKApplication on it); notifications requested at first launch on iPhone and iPad (the phone also notifies when an Apple Watch report arrives), opt-in via an in-context invitation elsewhere; a foreground presenter shows banners while the app is open. Not yet observed: a real background wake (needs hours on a device) and a notification landing on hardware. (§6.5, §10)
...
4. ~~Whether the EMTE logical tag lands in pointer bits 59:56 on Apple's implementation as on reference Arm MTE.~~ Yes: bits 59:56 carry 16 distinct values on the M5 Mac under checked allocations (Phase 9, `docs/evidence/self-test-Mac17,7-25G83.md`).
